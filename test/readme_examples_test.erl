...
direct_agent_and_provider_error() ->
    ...
    try
        ...
        ?assert(lists:any(fun
            (#{role := system,
               content := <<"You are a concise weather assistant.">>}) -> true;
            (_) -> false
        end, History)),
...
correlated_async_delegation() ->
    ...
    try
        ...
        receive
            {agent_response, Ref, AgentPid,
             {ok, <<"OTP uses supervised processes">>}} -> ok;
...
sync_and_async_runner() ->
    ...
    try
        ...
        {ok, StreamPid} = adk_runner:run_async(
                            Runner, UserId, AsyncSessionId,
                            <<"Explain supervisors.">>),
        try
            {ok, AsyncEvents} = drain_runner(StreamPid, []),
            ?assertEqual([<<"user">>, AgentName],
...
callbacks_and_telemetry() ->
    ...
    ok = telemetry:attach(
           HandlerId, [erlang_adk, agent, prompt, stop],
           TelemetryHandler, #{}),
    {ok, AgentPid} = erlang_adk:spawn_agent(
                       AgentName,
                       #{provider => adk_llm_probe,
    ...
    try
        ...
        receive {before_model, 0} -> ok
        after 1000 -> ?assert(false)
        end,
        receive {after_model, {ok, <<"callback response">>}} -> ok
        after 1000 -> ?assert(false)
        end,
        receive
