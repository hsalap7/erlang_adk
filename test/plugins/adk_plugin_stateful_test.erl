...
adapter_forwards_every_plugin_hook_test() ->
    {ok, Supervisor} = adk_plugin_runtime_sup:start_link(),
    unlink(Supervisor),
    {ok, Instance} = adk_plugin_runtime_sup:start_instance(
                       Supervisor,
                       #{id => <<"echo">>,
                         ...
                         config => #{}}),
    Hooks = [on_user_message, before_run, after_run,
             before_agent, after_agent,
             before_model, after_model, on_model_error,
             before_tool, after_tool, on_tool_error,
             on_event, on_agent_error, on_run_error, on_error],
    try
        lists:foreach(
          fun(Hook) ->
              Context = #{hook => Hook},
              ...
              Value = {value, Hook},
              ?assertEqual(
                 {amend, {Hook, NormalizedContext, Value}},
                 apply(adk_plugin_stateful_adapter, Hook,
                       [Context, Value,
                        #{instance => Instance, timeout_ms => 1000}]))
          end, Hooks),
        ?assertError(
           {stateful_plugin_failed,
            invalid_plugin_instance_invocation},
           adk_plugin_stateful_adapter:before_run(
             #{}, value, #{instance => Instance, timeout_ms => 0})),
        Dead = spawn(fun() -> ok end),
        DeadMonitor = erlang:monitor(process, Dead),
        receive
            {'DOWN', DeadMonitor, process, Dead, normal} -> ok
        after 1000 ->
            erlang:error(dead_plugin_fixture_timeout)
        end,
        ?assertError(
           {stateful_plugin_failed, instance_unavailable},
           adk_plugin_stateful_adapter:before_run(
             #{}, value, #{instance => Dead, timeout_ms => 1000}))
    after
        ok = adk_plugin_runtime_sup:stop_instance(Supervisor, Instance),
        exit(Supervisor, shutdown)
    end.
