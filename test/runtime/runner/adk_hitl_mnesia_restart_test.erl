...
paused_continuation_survives_mnesia_restart_test() ->
    ...
    try
        ...
        %% Separate the persistence contract from restart recovery: the pause
        %% is not observable until its continuation transaction has committed.
        %% Flush Mnesia's transaction log before simulating a node restart so
        %% this test cannot depend on the background log-dump schedule.
        {ok, BeforeRestart} = erlang_adk_session_mnesia:get_session(
                                ?APP, ?USER, ?SESSION),
        ?assert(maps:is_key(
                  <<"__adk_runner_continuation:", InvocationId/binary>>,
                  maps:get(state, BeforeRestart))),
        ok = mnesia:sync_log(),
