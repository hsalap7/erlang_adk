...
%% encode admitted client actions into one or more ordered frames, and decode
%% each provider frame into an ordered list of provider-neutral event
%% specifications. They do not own sockets or process lifetimes.
...
-type client_frames() :: binary() | nonempty_list(binary()).

-export_type([client_action/0, client_frames/0, event_spec/0]).
...
-callback encode_client(client_action(), map()) ->
    {ok, client_frames()} | ignored | {error, term()}.
