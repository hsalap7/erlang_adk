                    ...
                    case validate_public_route_mix(Config2) of
                        ok ->
                            case validate_dev_exposure(Config2) of
                                ok ->
                                    case validate_legacy_exposure(Config2) of
                                        ok ->
...
%% The project-specific legacy prompt route has no authentication contract.
%% Keep it useful for local compatibility without allowing a configuration
%% change to turn it into an unauthenticated, billable public endpoint.  The
%% public A2A v1 listener has its own authenticated TLS policy below.
validate_legacy_exposure(#{a2a_enabled := true, ip := Ip}) ->
    case is_loopback(Ip) of
        true -> ok;
        false -> {error, legacy_a2a_server_must_bind_loopback}
    end;
validate_legacy_exposure(_Config) -> ok.
