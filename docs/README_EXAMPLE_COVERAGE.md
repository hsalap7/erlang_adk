...
v0.6 clean gate passes 899 EUnit tests and six deterministic Common Test cases
with no failures, and Dialyzer reports no warnings across 170 project files.
...
trusted-proxy and direct-TLS modes also pass. The clean command did not enable paid
provider tests, so all 16 live cases were skipped there and are not counted as
passes. The checked-in
...
| --- | --- | --- | --- | --- | --- |
| F01 | Installation: Git dependency declaration | Erlang | Companion syntax | The project itself is compiled by the complete Rebar3 gate; no separate consumer project is created. | Requires Git/network access and the `version_0.6.0` branch. The literal `deps` term belongs in a consumer `rebar.config`. |
...
| F03 | Installation: export `GEMINI_API_KEY` | Bash | Live | `readme_live_gemini_SUITE:init_per_suite/1` verifies that the opt-in flag and key reach the test process. | Must run in the same shell as F77/F78. The key is never supplied to deterministic tests. |
...
| F72 | Configure and start the checked Phoenix companion locally | Bash | Adapter-dependent; Dedicated fixture | The checked `examples/phoenix_adk_ui` project is compiled and tested by its locked Mix gate; Erlang gateway behavior is covered by `adk_web_gateway_test`. | Requires a real OIDC client and F03 for Gemini execution. The callback must exactly match the loopback URI; public clients omit the secret but still require S256 PKCE. |
| F73 | Phoenix format, compile, test, assets, release, and dependency audit | Bash | Repository deterministic | Run inside `examples/phoenix_adk_ui`: format check, warnings-as-errors compile, 46 ExUnit tests, production assets, production release, and loopback `/health` boot checks in trusted-proxy and direct-TLS modes pass. `mix hex.audit` is always run and its exit status is recorded. | The lock pins the official LiveView security fix. The current audit exits non-zero for two documented Cowlib 2.18.0 advisories; this is a visible release exception, not a pass or a waived gate. |
...
| ID | README fence | Language | Classification | Exact validation | Prerequisites and continuation notes |
| --- | --- | --- | --- | --- | --- |
| F75 | Complete deterministic Erlang release and packaging gates | Bash | Literal deterministic | `./rebar3 do clean, compile, eunit, ct, dialyzer`; `./rebar3 escriptize`; `adk doctor`; checked agent-config validation. F63 covers the CLI-specific assertions. | Passed on 2026-07-14: 899 EUnit, six deterministic Common Test cases, warning-free Dialyzer over 170 project files, and all packaging checks. No live-provider flag is required. |
| F76 | Focused README and 1,000-run stress gates | Bash | Literal deterministic | `readme_examples_test`; `readme_workflow_examples_test`; `adk_concurrency_stress_SUITE`; `adk_v05_stress_SUITE` | Passed on 2026-07-14: 29 README tests, four workflow tests, one 1,000-run concurrency case, and both v0.5 resource/context stress cases. |
| F77 | Opt-in live Gemini suite | Bash | Live | `readme_live_gemini_SUITE`, including `context_cache/1` and `artifact_and_memory_tools/1` | Requires F03 and `ERLANG_ADK_LIVE_GEMINI=1` in the same shell, network access, quota, and billable API permission. Provider quota/rate failures are reported and never counted as passes. |
| F78 | Live Gemini suite without request pacing | Bash | Live | `readme_live_gemini_SUITE`; pacing is parsed by `request_interval_ms/0` | Intended only for projects whose quota supports unpaced requests. Keep the default interval on constrained projects. |
