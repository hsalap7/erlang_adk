...
passes 654 EUnit tests, four deterministic Common Test scenarios, and Dialyzer
over 134 project files; the two focused README modules pass all 33 tests, and
the packaging commands below also pass. The separate 14-scenario live Gemini
gate passes 13 cases, with Google Search grounding failing explicitly after
its bounded HTTP 429 retry. Live evidence supplements rather than replaces the
deterministic contracts in this ledger.
...
| --- | --- | --- | --- | --- | --- |
| F01 | Installation: Git dependency declaration | Erlang | Companion syntax | The project itself is compiled by the complete Rebar3 gate; no separate consumer project is created. | Requires Git/network access and the `version_0.4.0` branch. The literal `deps` term belongs in a consumer `rebar.config`. |
...
| F69 | Phoenix LiveView stable-run integration | Elixir | Companion syntax; Conceptual continuation | Public API behavior is covered by `adk_run_test`, `adk_agent_behavior_test`, `adk_sub_agents_test`, `adk_event_test`, `adk_admission_control_test`, `adk_runtime_policy_test`, and `adk_dev_http_test:ui_uses_typed_confirmation_payload_case/1`; the Elixir module is not compiled here. | Requires a supervised `PhoenixAgent` and authenticated server-side `user_id`. The documented credit/ack subscription, replay-gap handling, bounded UI history, and pause-type-specific resume payload must stay aligned with the listed tests. Unknown pause types should fail closed in a production companion. |
