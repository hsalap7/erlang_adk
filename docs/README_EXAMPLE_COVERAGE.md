...
final v0.7 clean Erlang gate passes 1,040 EUnit tests, six deterministic Common
Test cases, and warning-free Dialyzer analysis over 206 project files. Escript
packaging, CLI checks, 29 README tests, four workflow tests, three example
runtime smokes, 156 focused v0.7 tests, and both 1,000-run stress suites pass.
...
The final paid runs pass all four Gemini Live cases and 15 of 17 REST cases
...
| --- | --- | --- | --- | --- | --- |
| F01 | Installation: Git dependency declaration | Erlang | Companion syntax | The project itself is compiled by the complete Rebar3 gate; no separate consumer project is created. | Requires Git/network access and the `version_0.7.0` branch. The literal `deps` term belongs in a consumer `rebar.config`. |
...
| F06 | Google Search grounding request | Erlang | Deterministic equivalent; Billable REST | `adk_llm_gemini_test:test_google_search_grounding/1`, `test_grounded_tool_call_event/1`; `readme_live_gemini_SUITE:google_search_grounding/1` | REST Search use is model-selected and needs F03. The deterministic fixture proves request shape and metadata decoding. The 2026-07-14 REST case received HTTP 429 after one bounded retry, so it failed as rate-limited and is not counted as passed. |
...
| F57 | Legacy `/a2a/prompt` endpoint | Erlang | Deterministic equivalent; Billable REST | `erlang_adk_tests`; `erlang_adk_startup_test:supervised_http_is_bounded/0`; `readme_live_gemini_SUITE:http_endpoint/1` | REST execution needs F03. Tests use loopback and isolated ports. |
...
| F72 | Configure and start the checked Phoenix companion locally | Bash | Adapter-dependent; Dedicated fixture | The checked `examples/phoenix_adk_ui` project is compiled and tested by its locked Mix gate; Erlang gateway behavior is covered by `adk_web_gateway_test`; v0.7 Live/evaluation/observability projections are covered by `live_gateway/local_test.exs`, `live_projection_test.exs`, and `live_session_live_test.exs`. | Requires a real OIDC client and F03 for Gemini REST execution. Live discovery/control additionally requires a same-BEAM, server-owned session and the exact `adk.live.read`/`adk.live.control` scopes; observability/evaluation require their own read scopes. The browser supplies none of the provider/principal/module/path configuration. |
| F73 | Phoenix format, compile, test, assets, release, and dependency audit | Bash | Repository deterministic | On v0.7, format, warnings-as-errors compilation, all 61 ExUnit tests, production assets, and production release assembly pass. The packaged release returns HTTP 200 from loopback `/health` in trusted-proxy and certificate-verified direct-TLS modes, then stops cleanly. `mix hex.audit` must always run and retain its status. | The current audit is non-zero only for the two documented Cowlib 2.18.0 advisories; the exception remains visible. |
...
## v0.7 Live, plugin, observability, and evaluation additions

F79-F88 are stable IDs assigned to fences added in v0.7. Their section names
...
| ID | README fence | Language | Classification | Exact validation | Prerequisites and continuation notes |
| --- | --- | --- | --- | --- | --- |
| F75 | Complete deterministic Erlang release and packaging gates | Bash | Literal deterministic | `./rebar3 do clean, compile, eunit, ct, dialyzer`; `./rebar3 escriptize`; `adk doctor`; checked agent-config validation. F63 covers the CLI-specific assertions. | The final v0.7 gate passes 1,040 EUnit tests, six deterministic Common Test cases, warning-free Dialyzer over 206 project files, escript packaging, doctor, and checked config validation. No provider flag is required. |
| F76 | Focused README and 1,000-run stress gates | Bash | Literal deterministic | `readme_examples_test`; `readme_workflow_examples_test`; the focused v0.7 module set below; `adk_concurrency_stress_SUITE`; `adk_v05_stress_SUITE` | Current documentation checks pass 29 README tests, four workflow tests, warning-as-error compilation/runtime smoke for all three new example modules, 156 focused v0.7 tests, and both 1,000-run stress suites. No provider key is required. |
...
| F87 | Opt-in paid Gemini Live WebSocket suite | Bash | Gemini Live | `gemini_live_SUITE`: text/audio output plus transcription, PCM input, PNG image input, and synchronous tool round trip | Requires F03 and `ERLANG_ADK_GEMINI_LIVE=1` in the same shell, network access, quota, and billable API permission. The complete four-case suite passed against `gemini-3.1-flash-live-preview` during v0.7 development; a future provider failure must be reported rather than converted into a skip/pass. |
