...
This is the release feature inventory for the Erlang ADK 0.8.0 release
candidate. It preserves behavior proved by the final v0.7 release gates and
marks the added 0.8 provider/profile/Realtime slice separately. The final v0.8
deterministic, coverage, package, and Phoenix gates are recorded below; the
Gemini provider attempts failed externally at credential authentication and
are not treated as implementation passes or regressions.
...
- **Partial**: useful, release-safe core behavior exists, but this upstream
  ...
  necessarily blockers for the Erlang 0.8.0 contract.
- **In progress**: an implementation slice is present or being developed, but
  ...
  made yet.
- **Planned**: the behavior contract is identified for 0.8.0, but it is not
  implemented yet.
...
- **Deferred experimental**: the corresponding upstream feature is
  experimental or outside the 0.8.0 runtime contract and is not represented as
  silently supported.
...
A 0.8 addition is marked **Implemented** only when its public contract and
dedicated deterministic coverage passed the complete gates in
[`TESTING.md`](TESTING.md). A fixture is not evidence that a paid remote model
or arbitrary compatible endpoint works.
...
| Capability family | 0.8.0 status | Erlang-native contract |
| --- | --- | --- |
...
| Input/output schemas and structured output | Implemented | Inputs are rejected before provider execution; final output and callback replacements are validated before an `output_key` delta is committed in the same final event. The opaque session-service reference follows the caller scope, so the write targets the invocation session rather than the reusable agent's configured default. Gemini and OpenAI use their native checked schema forms, Anthropic uses GA `output_config.format`, and compatible endpoints use an explicit response-format mode which an operator profile can lock to `unsupported`. Remote compatible behavior still needs endpoint-specific evidence. |
| Provider-neutral generation configuration | Implemented | Common generation options are validated, normalized, and checked against discovered provider capabilities instead of being silently ignored. Gemini retains strict nested generation/safety/Search behavior. In 0.8, bounded binary provider/model aliases resolve operator-owned adapter, endpoint, credential, API/auth/privacy settings, and capability ceilings; caller allowlists retain only adapter-specific inference/runtime options. Direct atom-module configuration remains a trusted-code compatibility path. |
...
| Production web gateway | Implemented | `adk_scope_authorizer` validates issuer-bound principals and exact operation scopes. `adk_web_gateway` retains the server-owned stable-run catalog and owner scope. The Phoenix `LiveGateway` separately authorizes `adk.live.read`, `adk.live.control`, `adk.observability.read`, and `adk.evaluation.read`, discovers only server-owned same-BEAM sessions/reports, uses future-only credit/ack, removes media bytes/signatures before LiveView assigns, and rejects browser-selected principals/modules/paths. Its exact-origin binary voice socket revalidates the opaque web session per frame and uses one bounded core bridge per connection. In 0.8 the browser waits for a trusted format frame and resamples capture to Gemini's 16 kHz or OpenAI Realtime's 24 kHz PCM input, while preserving native playback, exact output ACKs, and interruption. Exact-scope artifact/memory/context administration, Live session creation/configuration, evaluation execution, and durable audit/revocation remain separate privileged APIs/adapters. |
| Command line and API server | Implemented | Authenticated REST/SSE plus `run`, `serve`, scoped `inspect` (including context lifecycle, Live and observability), exact cache-scope invalidation, Live text send, artifact delete, memory search/erase, `cancel`, `resume`, legacy `evaluate`, versioned `eval run`, `config validate`, and `doctor` are packaged in the `adk` escript. `config validate` and `doctor` recognize native OpenAI, Anthropic, compatible, and binary profile selections without printing credentials. `eval run` supports repeats, baseline CI gates, JSON/Markdown, and exit codes 0/2/1. Destructive calls require an exact matching confirmation object. |
| Visual workflow builder | Deferred experimental | The upstream visual builder and its Agent Config format are experimental. Erlang ADK 0.8.0 provides inspectable developer/Phoenix surfaces and declarative workflow APIs, but does not claim drag-and-drop code generation. |
...
| Agent skills | Deferred experimental | Upstream Skills are experimental. A future adapter must provide incremental discovery/loading plus an explicit filesystem/remote trust policy; 0.8.0 does not reinterpret ordinary prompts or MCP resources as Skills. |
...
| Outbound OAuth/API-key/bearer credentials | Implemented | Existing private per-principal tool/API credentials, immutable auth profiles, RFC 8707 targeting, bounded refresh/rotation, and secret-free status/errors remain. Separately, 0.8 model `provider_profiles` bind a binary alias to an adapter, endpoint, concrete model, locked options, and env/application/literal/none credential source; an opaque keyed profile snapshot makes credential lookup generation-consistent. Native ambient OpenAI/Anthropic keys are bound to exact official origins, and authenticated compatible endpoints require profile-materialized explicit credentials. Durable KMS/HSM storage, provider revocation, and distributed profile rollout remain adapters. |
...
| Model providers and routing | Implemented / Partial | Binary profiles select native Gemini, OpenAI Responses, Anthropic Messages, or an explicitly configured OpenAI-compatible Chat Completions adapter without allowing untrusted module/URL/header selection. Native adapters preserve their own content/tool/stream/error semantics; compatible structured output can be locked off and every target still requires endpoint-specific evidence. There is no automatic fallback, discovery, cost/latency routing, retry across vendors, Ollama/vLLM-specific contract, or managed-hosted adapter implied by this slice. |
...
bounded owner-bound bridge per browser voice connection. Its 0.8 format frame
