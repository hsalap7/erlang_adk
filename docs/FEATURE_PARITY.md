...
This is the release feature inventory for Erlang ADK 0.9.0, released on
2026-08-06. It preserves behavior proved by the final v0.8 release gates and
marks the added 0.9 graph/durability/model-fabric slice separately. The final
v0.8 deterministic, coverage, package, and Phoenix gates are historical
evidence below. The bounded v0.9 evidence is recorded in
[`VERSION_0_9_0.md`](VERSION_0_9_0.md); deterministic coverage is not presented
as paid-provider or arbitrary-deployment evidence.
...
- **Partial**: useful, release-safe core behavior exists, but this upstream
  ...
  necessarily blockers for the Erlang 0.9.0 contract.
- **In progress**: an implementation slice is present or being developed, but
  ...
  made yet.
- **Planned**: the behavior contract is identified for 0.9.0, but it is not
  implemented yet.
...
- **Deferred experimental**: the corresponding upstream feature is
  experimental or outside the 0.9.0 runtime contract and is not represented as
  silently supported.
...
A 0.9 addition is marked **Implemented** only when its public contract and
dedicated deterministic coverage exist in the released source. The recorded
release evidence is summarized in [`VERSION_0_9_0.md`](VERSION_0_9_0.md); a
fixture is not evidence that a paid remote model or arbitrary compatible
endpoint works.
...
| Capability family | 0.9.0 status | Erlang-native contract |
| --- | --- | --- |
...
| Command line and API server | Implemented | Authenticated REST/SSE plus `run`, `serve`, scoped `inspect` (including context lifecycle, Live and observability), exact cache-scope invalidation, Live text send, artifact delete, memory search/erase, `cancel`, `resume`, evaluation, `config validate`, and `doctor` are packaged in the `adk` escript. Local `graph validate`, `graph describe`, and `graph render` inspect an already available zero-arity graph factory as JSON, DOT, or Mermaid without unbounded atom creation. Destructive calls require an exact matching confirmation object. |
